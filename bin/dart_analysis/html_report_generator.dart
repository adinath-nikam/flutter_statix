...
<div class="header">
  ...
  <div class="subtitle">
    Generated on ${_formatTimestamp(timestamp)} | Dart $dartVersion
  </div>
</div>''';
}
...
String _generateFooter() {
  return '''
<div class="footer">
  <p>Generated by Flutter Statix • <a href="https://dart.dev/tools/dart-analyze" target="_blank">Learn about Dart analysis</a></p>
</div>''';
}
...
void _printSummary(Map<String, int> stats) {
  print('\n📊 Summary:');
  print('   Total Issues: ${stats['total']}');
  if ((stats['CRITICAL'] ?? 0) > 0) print('   🔴 Critical: ${stats['CRITICAL']}');
  if ((stats['ERROR'] ?? 0) > 0) print('   ❌ Errors: ${stats['ERROR']}');
  if ((stats['WARNING'] ?? 0) > 0) print('   ⚠️  Warnings: ${stats['WARNING']}');
  if ((stats['MAJOR'] ?? 0) > 0) print('   🟠 Major: ${stats['MAJOR']}');
  if ((stats['MINOR'] ?? 0) > 0) print('   🟡 Minor: ${stats['MINOR']}');
  if ((stats['INFO'] ?? 0) > 0) print('   🔵 Info: ${stats['INFO']}');
}